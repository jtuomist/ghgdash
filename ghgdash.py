...
# +
population_tab_content = dbc.Container([
...
buildings_tab_content = dbc.Container([
...
district_heat_tab_content = dbc.Row([
    dbc.Col([
...
def display_page(current_path):
    ...
    pages = [
        dict(name='Päästöt', path='', content=emissions_page),
        dict(name='Kaukolämpö', path='kaukolampo', content=district_heat_tab_content),
        dict(name='Sähkö', path='sahko'),
        dict(name='Liikenne', path='liikenne'),
        dict(name='Väestö', path='vaesto', content=population_tab_content),
        dict(name='Rakennuskanta', path='rakennukset', content=buildings_tab_content),
    ]
    navitems = []
    current_page = None
    for page in pages:
        attr = {}
        if current_path is not None and current_path == page['path']:
            attr['active'] = True
            current_page = page
        item = dbc.NavItem(dbc.NavLink(page['name'], href='/%s' % page['path'], **attr))
        navitems.append(item)

    if current_page is not None:
        page_content = html.Div([
            html.H2(current_page['name']),
            current_page.get('content')
        ])
    else:
